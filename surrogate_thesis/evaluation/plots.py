...
def plot_prediction_overview(
    ...
    axis.plot(ts, y_true[subset, 0], label="ground_truth", linewidth=2.2, color="black")
    for name, values in predictions_by_model.items():
        axis.plot(ts, values[subset, 0], label=name, alpha=0.9)
    # Figure context belongs in the thesis caption; retain only descriptive axes.
    axis.set_xlabel("Timestamp", fontsize=THESIS_FONT_SIZE)
    axis.set_ylabel("Load (kW)", fontsize=THESIS_FONT_SIZE)
    axis.tick_params(axis="both", labelsize=THESIS_FONT_SIZE)
    axis.legend(fontsize=THESIS_FONT_SIZE)
    fig.tight_layout()
    fig.savefig(output)
    plt.close(fig)
...
def plot_residual_overview(
    ...
    for axis, (name, residuals) in zip(axes[:, 0], residuals_by_model.items(), strict=False):
        ...
        # The model name identifies the panel without adding an in-figure title.
        axis.set_xlabel("Residual (kW)", fontsize=THESIS_FONT_SIZE)
        axis.set_ylabel(f"{name}\nDensity", fontsize=THESIS_FONT_SIZE)
        axis.tick_params(axis="both", labelsize=THESIS_FONT_SIZE)
    fig.tight_layout()
    fig.savefig(output)
    plt.close(fig)
...
def plot_runtime_comparison(metrics_frame: pd.DataFrame, output_path: str | Path) -> None:
    ...
    ylabel = "Runtime (ms)"
    title = "Model Runtime vs. Reference Simulator Runtime (Log Scale)"
    if metric_column == "latency_ms":
        ylabel = "Latency (ms)"
        title = "Average Inference Latency vs. Simulator Step Runtime (Log Scale)"
    # Full-test runtimes span several orders of magnitude. A logarithmic axis
    # keeps fast baselines visible alongside the neural models and simulator.
    plt.yscale("log")
    plt.ylabel(ylabel)
    plt.title(title)
    plt.grid(axis="y", which="both", linestyle="--", linewidth=0.6, alpha=0.45)
    plt.legend()
    plt.tight_layout()
