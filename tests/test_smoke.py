...
class SmokePipelineTests(unittest.TestCase):
    def test_smoke_pipeline_runs_end_to_end(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue((Path(temp_dir) / "run" / "plots" / "prediction_overview.pdf").exists())
            self.assertTrue((Path(temp_dir) / "run" / "plots" / "residuals.pdf").exists())
            self.assertTrue((Path(temp_dir) / "run" / "plots" / "normalized_error_comparison.png").exists())
            ...
            self.assertIn("MAPE", metrics_text)
            self.assertIn("NMAE", metrics_text)
            self.assertIn("NRMSE", metrics_text)
            ...
            self.assertIn("decorator_nmae", decorator_text)
            self.assertIn("decorator_nrmse", decorator_text)
